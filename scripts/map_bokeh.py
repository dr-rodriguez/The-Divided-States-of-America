...
p = figure(title="", toolbar_location="left", plot_width=1100, plot_height=700,
           x_axis_label='Longitude (degrees)', y_axis_label='Latitude (degrees)')
for code in states:
...
data = pd.DataFrame({'lat': lat, 'lon': lon, 'text': s.tweets['text'],
...
tooltip = [("Text", "@text"), ("Prediction", "@predict"), ("State", "@state")]
...
print('Trump Matches')
print s.tweets[s.tweets['predict'] == 1]['text'].sample(5, random_state=seed)  # Trump

# Compare with some states as well
print('Trump Matches in New York')
print s.tweets[(s.tweets['predict'] == 1) & (s.tweets['state'] == 'New York')]['text'].sample(5, random_state=seed)  # Trump
